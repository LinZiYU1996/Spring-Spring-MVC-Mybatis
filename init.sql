...
COMMIT;
 ...
 `id`      '用户id',
  ...
  `password`     '加密后的密码',
  `dept_id`      '用户所在部门的id',
  `status`       '状态，1：正常，0：冻结状态，2：删除',
  `remark`       '备注',
  `operator`      '操作者',
  `operate_time`       '最后一次更新时间',
  `operate_ip`      '最后一次更新者的ip地址',
  PRIMARY KEY (`id`)
  ...
  `parent_id`     '上级权限模块id',
  `level`      '权限模块层级',
  `seq`      '权限模块在当前层级下的顺序，由小到大',
  `status`      '状态，1：正常，0：冻结',
  `remark`     '备注',
  `operator`    '操作者',
  `operate_time`    '最后一次操作时间',
  `operate_ip`     '最后一次更新操作者的ip地址',
  PRIMARY KEY (`id`)
  ...
  `code`        '权限码',
  `name`     '权限名称',
  `acl_module_id`      '权限所在的权限模块id',
  `url`      '请求的url, 可以填正则表达式',
  `type`     '类型，1：菜单，2：按钮，3：其他',
  `status`     '状态，1：正常，0：冻结',
  `seq`   '权限在当前模块下的顺序，由小到大',
  `remark`    '备注',
  `operator`     '操作者',
  `operate_time`     '最后一次更新时间',
  `operate_ip`     '最后一个更新者的ip地址',
  PRIMARY KEY (`id`)
   ...
   `id`  '角色id',
  `name` ,
  `type`  '角色的类型，1：管理员角色，2：其他',
  `status`  '状态，1：可用，0：冻结',
  `remark`  '备注',
  `operator` '操作者',
  `operate_time`'最后一次更新的时间',
  `operate_ip` '最后一次更新者的ip地址',
  PRIMARY KEY (`id`)
  ...
  `operator`  '' COMMENT '操作者',
  `operate_time`  '最后一次更新的时间',
  `operate_ip` '最后一次更新者的ip',
  PRIMARY KEY (`id`)


  `id` 
  `type`  '权限更新的类型，1：部门，2：用户，3：权限模块，4：权限，5：角色，6：角色用户关系，7：角色权限关系',
  `target_id` '基于type后指定的对象id，比如用户、权限、角色等表的主键',
  `old_value` '旧值',
  `new_value` '新值',
  `operator` '''操作者',
  `operate_time`'最后一次更新的时间',
